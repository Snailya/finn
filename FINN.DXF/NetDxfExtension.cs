...
public static class NetDxfExtension
{
    ...
    public static void TransformBy(this EntityObject entity, TransformScale transformScale, Vector2d translate)
    {
        switch (entity)
        {
            case Hatch hatch:
                hatch.BoundaryPaths.SelectMany(x => x.Entities).ToList().ForEach(x =>
                    x.TransformBy(Matrix3.Scale(transformScale.Factor.X, transformScale.Factor.Y, 1),
                        (transformScale.BasePoint * (new Vector2d(1, 1) - transformScale.Factor) + translate)
                        .ToVector3()));
                break;
            default:
                entity.TransformBy(Matrix3.Scale(transformScale.Factor.X, transformScale.Factor.Y, 1),
                    (transformScale.BasePoint * (new Vector2d(1, 1) - transformScale.Factor) + translate).ToVector3());
                break;
        }
    }
